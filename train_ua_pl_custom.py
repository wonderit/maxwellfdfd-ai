...
if __name__ == '__main__':
    ...
    # if args.rpo_type == 'pl':
    #     args.num_models = 2
    pseudolabel_size = list(map(int, args.pseudo_label_size.split(',')))
    ...
    if args.pseudo_label:
        al_type = al_type + f'_pl({args.pseudo_label_size})'
    ...
    for iter_i in range(ITERATION):
        ...
        if not args.is_active_random:
            ...
            if args.pseudo_label:
                ...
                if args.pseudo_label_size[m] > 0:
                    random_pseudo_label_indices = np.random.permutation(args.pseudo_label_size[m])
                    X_pr_avg_U = X_pr_avg_U[random_pseudo_label_indices]
                    U_x_pl = U_x[random_pseudo_label_indices]
